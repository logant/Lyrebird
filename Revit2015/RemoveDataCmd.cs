...
namespace LMNA.Lyrebird
{
    ...
    class RemoveDataCmd : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ICollection<ElementId> elemSet = commandData.Application.ActiveUIDocument.Selection.GetElementIds();
                int itemCount = 0;
                if (elemSet.Count > 0)
                {
                    ...
                    else
                    {
                        using (Transaction trans = new Transaction(commandData.Application.ActiveUIDocument.Document, "Lyrebird - Remove Data"))
                        {
                            ...
                            foreach (ElementId eid in elemSet)
                            {
                                Element e = commandData.Application.ActiveUIDocument.Document.GetElement(eid);
                                try
                                {
